...
function addRoom() {
	location.href="addOrGo.action?functionid="+$('#functionid').val();
}
function submit() {
	$('#form').submit();
}	
...
<body class="page-header-fixed" onload="paging_init()">
	<div class="page-container">
		...
		<div class="Bread_crumbs">
			<font class="pull-left">手术室列表</font> <img
		...
		<div class="responsive-table">
			...
			<form id="form" action="orList.action" name="form" method="post">
			...
			<div class="table_input " >
				...
				<input type="hidden" name="page" id="page" value="${page}" /> <input
					type="hidden" name="userid" id="id" value="${id}" /> 手术室名称：<input style="width: 120px;"
					name="sname" size="8" value="${sname}" />&nbsp;&nbsp; 手术室类型：<select style="width: 100px;"
					name="lx">
					<option></option>
					<option value="0" <c:if test="${lx eq '0'}">selected</c:if>>手术室</option>
					<option value="1" <c:if test="${lx eq '1'}">selected</c:if>>恢复室</option>
				</select>&nbsp;&nbsp; 有效标志：<select style="width: 100px;"
					name="yx">
					<option></option>
				...
				<c:if test="${addRole eq true }">	
					<button type="button" class="button_blue_74_33" onclick="addRoom()">新增</button>
				</c:if>	
				</div>
			</div>
				...
				<thead>
					...
					<c:forEach items="${orList}" var="sor">
						<tr>
							<td><input name="ids" type="hidden" value="${sor.id}" /><%=xh++%></td>
							<td>${sor.name}</td>
							<td><c:if test="${sor.roomType eq '0'}">手术室</c:if> <c:if
									test="${sor.roomType eq '1'}">恢复室</c:if>
							</td>

							<td><c:if test="${sor.enable eq '0'}">有效</c:if> <c:if
									test="${sor.enable eq '1'}">无效</c:if>
							</td>
							<c:if test="${updateRole eq true }">
							<td style="text-align:center;"><a
								href="modOrListTo.action?id=${sor.id}&functionid=${functionid}">修&nbsp;改</a></td>
							</c:if>	

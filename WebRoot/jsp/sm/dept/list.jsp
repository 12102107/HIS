...
function addDept() {
	location.href="addDeptGo.action?functionid="+$('#functionid').val();
}
function submit() {
	$('#form').submit();
}	
...
<body class="page-header-fixed" onload="paging_init()">
	<div class="page-container">
		...
		<div class="Bread_crumbs">
			<font class="pull-left">科室列表</font> <img
		...
		<div class="responsive-table">
			...
			<form id="form" action="deptList.action" name="form" method="post">
			...
			<div class="table_input " >
				...
				<input type="hidden" name="page" id="page" value="${page}" /> <input
					type="hidden" name="userid" id="id" value="${id}" /> 名称：<input style="width: 120px;"
					name="kname" size="8" value="${kname}" />&nbsp;&nbsp; 有效标志：<select style="width: 100px;"
					name="yx">
					<option></option>
				...
				<table class="table table-bordered table-striped header-spcing" id="table">
					...
					<c:forEach items="${deptList}" var="dept">
						<tr>
							<td><input name="ids" type="hidden" value="${dept.id}" /><%=xh++%></td>
							<td>${dept.name}</td>
							<td><c:if test="${dept.enable eq '0'}">有效</c:if> <c:if
									test="${dept.enable eq '1'}">无效</c:if>
							</td>
							<c:if test="${updateRole eq true }">
							<td style="text-align:center;"><a
								href="modDeptListTo.action?id=${dept.id}&functionid=${functionid}">修&nbsp;改</a>
							</c:if>	

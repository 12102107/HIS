...
function addDept() {
	location.href="addInstrumentPackageTo.action?functionid="+$('#functionid').val();
}
function submit() {
	$('#form').submit();
}
function delI(id) {
	if(confirm("确认删除？")) {
		$.post("delInstrumentPackage.action",{id:id},function(){
			alert("删除成功！");
			$('#form').submit();
		});
	}
...
<body class="page-header-fixed" onload="paging_init()">
	<div class="page-container">
		...
		<div class="Bread_crumbs">
			<font class="pull-left">手术包列表</font> <img
		...
		<div class="responsive-table">
			...
			<form id="form" action="getInstrumentPackageList.action" name="form" method="post">
				...
				<table class="table table-bordered table-striped header-spcing" id="table">
					...
					<c:forEach items="${list}" var="i" varStatus="s">
						<tr>
							...
							<a href="modInstrumentPackageTo.action?id=${i.id}&functionid=${functionid}">修改</a>
							&nbsp;&nbsp;

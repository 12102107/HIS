...
function addMedicine() {
	location.href="addMedGo.action?functionid="+$('#functionid').val();
}
function submit() {
	$('#form').submit();
}
...
<body class="page-header-fixed" onload="paging_init()">
	<div class="page-container">
		...
		<div class="Bread_crumbs">
			<font class="pull-left">药品列表</font> <img
		...
		<div class="responsive-table">
			...
			<form id="form" action="medList.action" name="form" method="post">
			...
			<div class="table_input " >
				...
				<input type="hidden" name="page" id="page" value="${page}" /> <input
					...
					name="code" size="8"  value="${code}"/>&nbsp;&nbsp; 药品名：<input name="ypname" style="width: 120px;"
					size="8" value="${ypname}"/>&nbsp;&nbsp; 药品分类：<select name="fl" style="width: 100px;">
					<option></option>
					<c:forEach items="${flList}" var="fl1" varStatus="status">
						<option value="${fl1}" <c:if test="${fl eq fl1}">selected</c:if>>${fl1}</option>
					</c:forEach>
				</select>&nbsp;&nbsp; 
				单位：<select name="dw" style="width: 100px;">
					<option></option>
				...
				<c:if test="${addRole eq true }">	
					<button type="button" class="button_blue_74_33" onclick="addMedicine()">新增</button>
				</c:if>	
				</div>
			</div>
			<table class="table table-bordered table-striped header-spcing" id="table">
				<thead>
					...
					<c:forEach items="${medList}" var="med">
						<tr>
							<td><input name="ids" type="hidden" value="${med.id}" /><%=xh++%></td>
							<td>${med.name}</td>
							<td>${med.briefName}</td>
							<td>${med.roughType}</td>
							<td>${med.type}</td>
							<td>${med.pinyin}</td>
							<td>${med.hismedicineCode}</td>
							<td>${med.packageDosageAmount}</td>
							<td>${med.dosageUnit}</td>
							<td><c:if test="${med.enable eq '0'}">有效</c:if> <c:if
									test="${med.enable eq '1'}">无效</c:if></td>
							<c:if test="${updateRole eq true }">
							<td style="text-align:center;"><a
								href="modMedListTo.action?id=${med.id}&functionid=${functionid}">修&nbsp;改</a></td>
							</td>
							</c:if>
						</tr>
					</c:forEach>

...
function addZd() {
	location.href="addDzGo.action?functionid="+$('#functionid').val();
}
function submit() {
	$('#form').submit();
}
...
<body class="page-header-fixed" onload="paging_init()">
	<div class="page-container">
		...
		<div class="Bread_crumbs">
			<font class="pull-left">诊断列表</font> <img
		...
		<div class="responsive-table">
			...
			<form id="form" action="zdList.action" name="form" method="post">
			...
			<div class="table_input " >
				...
				诊断名：<input name="zdname" style="width: 120px;"
					size="8" value="${zdname}"/>&nbsp;&nbsp; 
				拼音码：<input name="pinyin" size="8" value="${pinyin}" style="width: 120px;"/>&nbsp;&nbsp;
				...
				<c:if test="${addRole eq true }">	
					<button type="button" class="button_blue_74_33" onclick="addZd()">新增</button>
				</c:if>	
				</div>
			</div>
				...
				<thead>
					...
					<c:forEach items="${zdList}" var="zd">
						<tr>
							<td><input name="ids" type="hidden" value="${zd.id}" /><%=xh++%></td>
							<td>${zd.name}</td>
							<td>${zd.pinyin}</td>
							<td><c:if test="${zd.enable eq '0'}">有效</c:if> 
					    	 <c:if test="${zd.enable eq '1'}">无效</c:if></td>
					    	<c:if test="${updateRole eq true }">	
							<td style="text-align:center;"><a
								href="modZdListTo.action?id=${zd.id}&functionid=${functionid}">修&nbsp;改</a>
							</td>
							</c:if>
						</tr>
					</c:forEach>

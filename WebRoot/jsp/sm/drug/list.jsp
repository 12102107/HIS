...
function addDrug() {
	location.href="addDrugGo.action?functionid="+$('#functionid').val();
}
function submit() {
	$('#form').submit();
}
...
<body class="page-header-fixed" onload="paging_init()">
	<div class="page-container">
		...
		<div class="Bread_crumbs">
			<font class="pull-left">使用贵重或自费药品(物品)列表</font> <img
		...
		<div class="responsive-table">
			...
			<form id="form" action="drugList.action" name="form" method="post">
			...
			<div class="table_input " >
				...
				代码：<input name="code" style="width: 120px;"
					size="8" value="${code}"/>&nbsp;&nbsp;
				名称：<input name="dname" style="width: 120px;"
					size="8" value="${dname}"/>&nbsp;&nbsp; 
				类型：<input name="lx" size="8" style="width: 120px;" value="${lx}"/>&nbsp;&nbsp;
				...
				<c:if test="${addRole eq true }">	
					<button type="button" class="button_blue_74_33" onclick="addDrug()">新增</button>
				</c:if>	
				</div>
			</div>
				...
				<thead>
					...
					<c:forEach items="${drugList}" var="drug">
						<tr>
							<td><input name="ids" type="hidden" value="${drug.code}" /><%=xh++%></td>
							<td>${drug.code}</td>
							<td>${drug.name}</td>
							<td>${drug.price}</td>
							<td>${drug.type}</td>
							<td><c:if test="${drug.enable eq '0'}">有效</c:if> 
					    	 <c:if test="${drug.enable eq '1'}">无效</c:if></td>
					    	<c:if test="${updateRole eq true }">	
							<td style="text-align:center;"><a
								href="modDrugListTo.action?id=${drug.code}&functionid=${functionid}">修&nbsp;改</a>
							</td>
							</c:if>

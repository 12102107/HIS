...
//验证手术名称列表名称
	function checkName(){
		...
		$.ajax({
	      type:"post",
	      url:"<%=request.getContextPath()%>/ss/checkSsName.action",
	      data:{
	      	name:name,
	      },
	      async:false,
...
//验证手术名称列表代码
	function checkCode(){
		...
		$.ajax({
	      type:"post",
	      url:"<%=request.getContextPath()%>/ss/checkSsCode.action",
	      data:{
	      	code:code,
	      },
	      async:false,
...
<body class="page-header-fixed">
	<div class="page-container">
		...
		<div class="Bread_crumbs">
			<font class="pull-left">添加手术</font> <img
		...
		<div class="responsive-table">
      	...
      	<form name="form" action="${pageContext.request.contextPath}/ss/addSs.action" method="post" id="form">
			...
			<div style="text-align: center; margin-top: 2%;">
				...
				<button type="button" class="button_blue_74_33"  style="border: medium none;" onclick="javascript:window.location.href = 'ssList.action?functionid=${functionid}'">取消</button>
			</div>		
		</form>
</div></div></div>
</body>

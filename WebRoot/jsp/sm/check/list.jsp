...
function addcheck() {
	location.href="addCheckGo.action?functionid="+$('#functionid').val();
}
function submit() {
	$('#form').submit();
}
...
<body class="page-header-fixed" onload="paging_init()">
	<div class="page-container">
		...
		<div class="Bread_crumbs">
			<font class="pull-left">检验项目列表</font> <img
		...
		<div class="responsive-table">
			...
			<form id="form" action="checkList.action" name="form" method="post">
			...
			<div class="table_input " >
				...
				代码：<input name="code" style="width: 120px;"
					size="8" value="${code}"/>&nbsp;&nbsp;
				名称：<input name="cname" style="width: 120px;"
					size="8" value="${cname}"/>&nbsp;&nbsp; 
				单位：<input name="dw" style="width: 120px;" value="${dw}"/>&nbsp;&nbsp;
				...
				<c:if test="${addRole eq true }">	
					<button type="button" class="button_blue_74_33" onclick="addcheck()">新增</button>
				</c:if>	
				</div>
			</div>
				...
				<thead>
					...
					<c:forEach items="${checkList}" var="check">
						<tr>
							<td><input name="ids" type="hidden" value="${check.code}" /><%=xh++%></td>
							<td>${check.code}</td>
							<td>${check.name}</td>
							<td>${check.unit}</td>
							<td><c:if test="${check.enable eq '0'}">有效</c:if> 
					    	 <c:if test="${check.enable eq '1'}">无效</c:if></td>
					    	<c:if test="${updateRole eq true }">
							<td style="text-align:center;"><a
								href="modCheckListTo.action?id=${check.code}&functionid=${functionid}">修&nbsp;改</a>
							</td>
							</c:if>
						</tr>

...
//初始化时根据手术室类型查询手术室
	...
	function zx() {
		location.href = "${pageContext.request.contextPath}/gzz/portList.action?id="
				+ $("#id").val();
	}
</script>
</head>
		...
		<div class="responsive-table">
			<div class="scrollable-area">
				<form name="form"
					...
					<input type="hidden" name="id" id="id" value="${ws.id}"> <input
						type="hidden" name="ws.portId" value="${port.id}">
					<table class="table table-bordered table-striped header-spcing">
						<tr>
							<td bgcolor="#fff" style="vertical-align: middle;">工作站ID:<input
								...
								value="${ws.id}">
								<div id="gzzIddiv" class="div1"></div>
							</td>
						</tr>
						<tr>
							<td bgcolor="#fff">手术室类型:<select name="ws.type"
								style="vertical-align: middle;">
									<option value="0" class="room"
										<c:if test="${ws.type eq '0'}">selected</c:if>>手术室</option>
									<option value="1" class="room"
										<c:if test="${ws.type eq '1'}">selected</c:if>>恢复室</option>
							</select>
							</td>
						</tr>
						<tr>
							<td bgcolor="#fff">手术室:<select id="room" name="ws.room"
								style="vertical-align: middle;">
									<option class="room">${ws.room}</option>
							</select>
							</td>
						</tr>
						<tr>
						...
						<tr>
							<td bgcolor="#fff">监控服务器IP:<input type="text"
								name="ws.controlIp" value="${ws.controlIp}">
							</td>
						</tr>
						<tr>
							<td bgcolor="#fff">监控服务器端口:<input type="text"
								name="ws.controlPort" value="${ws.controlPort}">
							</td>
						</tr>
						<tr>
							<td bgcolor="#fff">是否有效:<select name="ws.enable"
								style="vertical-align: middle;">
									<option value="0"
										<c:if test="${ws.enable eq '0'}">selected</c:if>>有效</option>
									<option value="1"
										<c:if test="${ws.enable eq '1'}">selected</c:if>>无效</option>
							</select>
							</td>
						</tr>
						<tr>
							<td bgcolor="#fff">NET 端口:<input type="text" readonly="readonly"
								name="ws.netPort" value="${port.model}"> <a
								onclick="zx()">配置</a></td>
						</tr>
